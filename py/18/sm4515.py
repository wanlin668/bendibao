...
class Spider(BaseSpile):
    ...
    def playerContent(self, flag, id, vipFlags):
        try:
            ...
            header.update({"Referer": f"{self.host}/vid/{vid}.html" if re.fullmatch(r"\d+", vid or "") else self.host + "/", "Origin": self.host, "Connection": "keep-alive"})
            if url and not any(x in url.lower() for x in [".m3u8", ".mp4"]):
                ...
                for p in [r"let\s+bfz\s*=\s*['\"]([^'\"]+)", r"var\s+bfz\s*=\s*['\"]([^'\"]+)", r"video\s*:\s*\{[^}]*url\s*:\s*([^,}]+)", r"(https?://[^\s'\"]+\.m3u8[^\s'\"]*)"]:
