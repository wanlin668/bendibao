# -*- coding: utf-8 -*-
# 目标站: 神秘电影 (https://3e5bh.sm455.vip)
# 模板: 自建站 (XOR128 文本混淆 + DPlayer HLS)
# 功能: 分类浏览、详情解析、搜索、m3u8播放
import re
import json
import urllib.parse
from base.spider import Spider as BaseSpider
import requests
from bs4 import BeautifulSoup


class VideoDecryptor:
    """XOR 128 解密"""
    @staticmethod
    def decrypt(text):
        if not text:
            return ""
        try:
            return ''.join(chr(128 ^ ord(c)) for c in text)
        except:
            return text

    @staticmethod
    def from_js(js):
        m = re.search(r"document\.write\(l\('([^']+)'\)\)", js)
        return VideoDecryptor.decrypt(m.group(1)) if m else ""


class Spider(BaseSpider):

    def getName(self):
        return "新时代鉴赏"

    def init(self, extend=""):
        self.host = "https://3e5bh.sm455.vip"
        self.video_host = "https://38.je:38"
        self.image_host = "https://3334.nl:33"
        self.headers = {
            "User-Agent": "Mozilla/5.0 (Linux; Android 13; 22127RK46C Build/TKQ1.220905.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/104.0.5112.97 Mobile Safari/537.36",
            "Referer": self.host,
            "Accept-Language": "zh-CN,zh;q=0.9",
        }
        self.cache = {}

    def get(self, url):
        try:
            r = requests.get(url, headers=self.headers, timeout=15)
            r.raise_for_status()
            r.encoding = "utf-8"
            return r.text
        except:
            return ""

    def img_url(self, url):
        """格式化图片URL，内嵌UA和Referer用于TVBox防盗链处理"""
        if not url:
            return ""
        if url.startswith("//"):
            url = "https:" + url
        elif url.startswith("/"):
            url = self.image_host + url
        return f"{url}@User-Agent={self.headers['User-Agent']}@Referer={self.host}/"

    def parse_card(self, el):
        """解析视频卡片"""
        a = el if el.name == 'a' else el.find('a')
        if not a or not (href := a.get("href", "")):
            return None

        href = self.host + href if href.startswith("/") else href
        m = re.search(r"/vid/(\d+)", href)
        if not m:
            return None

        vid = m.group(1)
        title = ""

        # 解密标题
        if p := el.find('p'):
            if s := p.find('script'):
                if s.string:
                    title = VideoDecryptor.from_js(s.string)
            title = title or p.get_text(strip=True)

        if not title:
            for attr in ['data-title', 'data-name', 'title']:
                if el.has_attr(attr) and (val := el[attr]):
                    de = VideoDecryptor.decrypt(val)
                    if de and len(de) > 3:
                        title = de
                        break

        title = title or "未知标题"
        if title != "未知标题":
            self.cache[vid] = title

        # 图片
        img = ""
        if node := el.select_one("img"):
            img = node.get("data-src") or node.get("src") or ""
        img = img or f"{self.image_host}/{vid}.jpg"

        return {
            "vod_id": vid,
            "vod_name": title,
            "vod_pic": self.img_url(img),
            "vod_remarks": "",
        }

    def get_title(self, vid):
        """从缓存或首页获取标题"""
        if vid in self.cache:
            return self.cache[vid]

        html = self.get(self.host)
        if not html:
            return None

        soup = BeautifulSoup(html, "html.parser")
        for link in soup.select('a[href*="/vid/"]'):
            if f'/vid/{vid}' in link.get('href', ''):
                if p := link.find('p'):
                    if s := p.find('script'):
                        if s.string and (t := VideoDecryptor.from_js(s.string)):
                            self.cache[vid] = t
                            return t
                    if t := p.get_text(strip=True):
                        self.cache[vid] = t
                        return t
        return None

    def _parse_videos(self, html):
        """从HTML解析视频列表"""
        if not html:
            return []
        soup = BeautifulSoup(html, "html.parser")
        videos = [v for v in (self.parse_card(el) for el in soup.select(".vodbox, .stui-vodlist__box, .vodlist__box, .video-card, .item")) if v]

        if not videos:
            videos = [{
                "vod_id": v,
                "vod_name": "未知标题",
                "vod_pic": self.img_url(f"{self.image_host}/{v}.jpg"),
                "vod_remarks": ""
            } for v in re.findall(r'\[]\(/vid/(\d+)\.html\)', html)]

        return videos

    def homeContent(self, filter):
        return {
            "class": [
                {"type_name": "国产", "type_id": "1"},
                {"type_name": "日本", "type_id": "2"},
                {"type_name": "韩国", "type_id": "3"},
                {"type_name": "欧美", "type_id": "4"},
                {"type_name": "三级", "type_id": "5"},
                {"type_name": "动漫", "type_id": "6"},
            ],
            "filters": {}
        }

    def homeVideoContent(self):
        html = self.get(self.host)
        return {"list": self._parse_videos(html)}

    def categoryContent(self, tid, pg, filter, extend):
        if tid == "0":
            url = self.host if int(pg) == 1 else f"{self.host}/page/{pg}.html"
        else:
            url = f"{self.host}/list/{tid}.html" if int(pg) == 1 else f"{self.host}/list/{tid}/{pg}.html"

        html = self.get(url)
        if not html:
            return {"list": [], "page": pg, "pagecount": 1, "limit": 30, "total": 0}

        soup = BeautifulSoup(html, "html.parser")
        last = max(
            [int(m.group(1)) for a in soup.select("a[href*='list/']")
             if (m := re.search(r"/list/\d+/(\d+)\.html", a.get("href", "")))],
            default=int(pg)
        )

        return {"list": self._parse_videos(html), "page": pg, "pagecount": max(last, 1), "limit": 30, "total": 99999}

    def searchContent(self, key, quick, pg="1"):
        url = f"{self.host}/so.html"
        params = {"wd": key}
        if int(pg) > 1:
            params["page"] = pg

        html = ""
        for method in [requests.get, requests.post]:
            try:
                if method == requests.get:
                    r = method(url, params=params, headers=self.headers, timeout=15)
                else:
                    r = method(url, data=params, headers=self.headers, timeout=15)
                r.raise_for_status()
                r.encoding = "utf-8"
                html = r.text
                break
            except:
                continue

        if not html:
            return {"list": []}

        soup = BeautifulSoup(html, "html.parser")
        last = max(
            [int(m.group(1)) for a in soup.select("a[href*='so.html'], .pagination a, .page-link")
             if (m := re.search(r"[?&]page=(\d+)", a.get("href", "")))],
            default=int(pg)
        )

        return {"list": self._parse_videos(html), "page": pg, "pagecount": max(last, 1), "limit": 30, "total": 99999}

    def detailContent(self, ids):
        vid = ids[0]
        html = self.get(f"{self.host}/vid/{vid}.html")
        if not html:
            return {"list": []}

        soup = BeautifulSoup(html, "html.parser")

        # 标题
        title = self.get_title(vid)
        if not title:
            if t := soup.find('title'):
                title = re.sub(r'\s*[-_|]\s*.{0,20}$', '', t.get_text(strip=True)).strip()

            if not title or len(title) < 5:
                for sel in ['h1', 'h2', '.video-title', '.title']:
                    if (el := soup.select_one(sel)) and (txt := el.get_text(strip=True)) and len(txt) > 5:
                        title = txt
                        break

        title = title or f"视频{vid}"

        # 图片
        pic = ""
        for sel in ['.picbox img', '.vodimg img', '.video-pic img', '.poster img', 'img[data-id]']:
            if (node := soup.select_one(sel)) and (p := node.get("data-src") or node.get("src")) and 'favicon' not in p.lower():
                pic = p
                break

        if not pic or 'favicon' in pic.lower():
            if meta := soup.select_one('meta[property="og:image"]'):
                pic = meta.get('content', '')

        pic = pic or f"{self.image_host}/{vid}.jpg"

        # 简介
        desc = soup.select_one(".vodinfo, .video-info, .content, .intro, .description")
        desc = desc.get_text(strip=True) if desc else ""

        return {"list": [{
            "vod_id": vid,
            "vod_name": title,
            "vod_pic": self.img_url(pic),
            "vod_content": desc,
            "vod_play_from": "新时代鉴赏",
            "vod_play_url": f"在线播放${vid}@@0@@1",
        }]}

    def playerContent(self, flag, id, vipFlags):
        try:
            vid = id.split("@@")[0] if "@@" in id else id
            if vid.startswith("http") and any(x in vid.lower() for x in [".m3u8", ".mp4"]):
                url = vid
            elif re.fullmatch(r"\d+", vid or ""):
                url = f"{self.video_host}/{vid}/index.m3u8"
            else:
                url = vid

            header = dict(self.headers)
            header.update({
                "Referer": f"{self.host}/vid/{vid}.html" if re.fullmatch(r"\d+", vid or "") else self.host + "/",
                "Origin": self.host,
                "Connection": "keep-alive"
            })

            if url and not any(x in url.lower() for x in [".m3u8", ".mp4"]):
                html = self.get(url if url.startswith("http") else self.host + url if url.startswith("/") else url)
                for p in [r"let\s+bfz\s*=\s*['\"]([^'\"]+)", r"var\s+bfz\s*=\s*['\"]([^'\"]+)",
                          r"video\s*:\s*\{[^}]*url\s*:\s*([^,}]+)", r"(https?://[^\s'\"]+\.m3u8[^\s'\"]*)"]:
                    m = re.search(p, html, re.S)
                    if m:
                        raw = m.group(1).strip()
                        m2 = re.search(r"['\"]([^'\"]+\.m3u8[^'\"]*)['\"]", raw)
                        url = m2.group(1) if m2 else raw
                        break

            return {"parse": 0, "playUrl": "", "url": url, "header": json.dumps(header)}
        except Exception as e:
            return {"parse": 0, "playUrl": "", "url": "", "header": json.dumps(self.headers)}

    def localProxy(self, param):
        return {"code": 404, "content": ""}

    def isVideoFormat(self, url):
        return ".m3u8" in url.lower()

    def manualVideoCheck(self):
        pass

    def destroy(self):
        pass
